...
from weixin4auto.languages import MENU_OPTIONS
from weixin4auto.ui.component import Menu
from weixin4auto.utils.win32 import SetClipboardText
    ...
    def find_session_in_current_view(
        ...
        try:
            ...
            for session in visible_sessions:
                extracted_name = extract_name(session.name)
                
                if exact:
                    # 精确匹配
                    if (extracted_name == clean_keywords or 
                        extracted_name == keywords or
                        session.name == clean_keywords or 
                        session.name == keywords):
                        return session
                else:
                    # 模糊匹配
                    if (clean_keywords in extracted_name or 
                        keywords in extracted_name or
                        clean_keywords in session.name or 
                        keywords in session.name):
                        return session
...
class SessionElement:
    ...
    def __repr__(self):
        ...
        if len(content) > 5:
            content = content[:5] + '...'
        return f"<weixin4auto Session Element({content})>"
...
class SearchResultElement:
    ...
    def __repr__(self):
        ...
        if len(content) > 5:
            content = content[:5] + '...'
        return f"<weixin4auto Search Element({content})>"
